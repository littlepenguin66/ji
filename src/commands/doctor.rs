...
fn check_config(report: &mut DoctorReport) {
    ...
    if !cfg_path.exists() {
        report.checks.push(error(
            ...
            Some("run `ji init` to create".into()),
        ));
        return;
    }
    ...
    match crate::store::config::Config::read(&cfg_path) {
        Ok(cfg) => {
            ...
            if n == 0 {
                ...
                report
                    .checks
                    .push(ok("Config", &format!("{n} recipient(s) configured"), None));
            }
            ...
            if n_remotes == 0 {
                ...
                report.checks.push(ok(
                    "Config",
                    &format!("{n_remotes} remote(s) configured"),
                    None,
                ));
            }
        }
...
fn check_keys(report: &mut DoctorReport) {
    ...
    if identity.exists() {
        report.checks.push(ok(
            "Keys",
            "ji identity key found",
            Some(identity.display().to_string()),
        ));
    } else if ssh_count > 0 {
        report.checks.push(ok(
            "Keys",
            "no ji identity key, but SSH keys available",
            Some("age will use ~/.ssh keys for decryption".into()),
        ));
    } else {
        report.checks.push(warn(
            "Keys",
            "no ji identity key and no SSH keys",
    ...
    if identity_pub.exists() {
        report
            .checks
            .push(ok("Keys", "ji identity public key found", None));
    }
    ...
    if ssh_dir.exists() {
        if ssh_count > 0 {
            report.checks.push(ok(
                "Keys",
                &format!("{ssh_count} SSH private key(s) in ~/.ssh"),
                None,
            ));
        } else {
            report.checks.push(warn(
                "Keys",
                "no SSH private keys in ~/.ssh",
        ...
        report
            .checks
            .push(warn("Keys", "~/.ssh directory not found", None));
    }
...
fn count_ssh_keys(ssh_dir: &std::path::Path) -> u32 {
    ...
    if let Ok(entries) = std::fs::read_dir(ssh_dir) {
        for entry in entries.flatten() {
            ...
            if name.starts_with("id_") && !name.ends_with(".pub") && entry.path().is_file() {
                count += 1;
            }
        }
    }
...
fn check_manifest(report: &mut DoctorReport) {
    ...
    match crate::store::manifest::Manifest::read(&manifest_path) {
        Ok(manifest) => {
            ...
            report
                .checks
                .push(ok("Manifest", &format!("{n} file(s) tracked"), None));
            ...
            if !missing.is_empty() {
                ...
                report
                    .checks
                    .push(ok("Manifest", "all tracked files exist on disk", None));
            }
            ...
            match crate::store::manifest::compute_status(&manifest) {
                Ok(statuses) => {
                    ...
                    if !modified.is_empty() {
                        ...
                        report
                            .checks
                            .push(ok("Manifest", "all checksums up to date", None));
                    }
                }
                Err(e) => {
                    report.checks.push(error(
                        "Manifest",
                        "failed to compute status",
                        Some(e.to_string()),
                    ));
                }
            }
                    ...
                    match crate::archive::format::read_header(&mut file) {
                        Ok((cipher, index_len)) => {
                            ...
                            if file.read_exact(&mut index_buf).is_ok() {
                                match crate::archive::format::read_index(&mut std::io::Cursor::new(
                                    &index_buf,
                                )) {
                                    Ok(index) => {
                                        report.checks.push(ok(
...
mod tests {
    ...
    fn doctor_all_ok_with_valid_setup() {
        ...
        crate::store::path::with_test_home(tmp.path(), || {
            ...
            manifest
                .write(&crate::store::path::manifest_toml())
                .unwrap();
